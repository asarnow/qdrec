function [G,Gp_full] = nb_voting(svms,Z,flag,param)
% [G,Gp_full] = nb_voting(svms,Z,flag,param);
% Conduct majority voting between Naive Bayes (NB) classifiers.
% Input:
%  svms            cell array containing trained NB structs
...
%                  contains NBs which are parameterized by e.g. by subsets
...
if np == 1
    ...
    for j=1:np
        ...
        G{j} = zeros(n,1);
        for i=1:ms
            G{j} = G{j} + predict(svms{i,j},Z(mask,:));
        end
        G{j} = G{j} / ms;
