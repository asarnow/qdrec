function [G,Gp_full] = tb_voting(svms,Z,flag,param)
% [G,Gp_full] = tb_voting(svms,Z,flag,param);
% Conduct majority voting between random forest / TreeBagger classifiers.
% Input:
%  svms            cell array containing trained RF structs
...
%                  contains RFs which are parameterized by e.g. by subsets
...
if np == 1
    ...
    G = zeros(N,1);
    for i=1:length(svms)
        G = G + cellfun(@str2double,svms{i}.predict(Z));
    end
    G = G / m;
    ...
    for j=1:np
        ...
        G{j} = zeros(n,1);
        for i=1:ms
            G{j} = G{j} + cellfun(@str2double,svms{i,j}.predict(Z(mask,:)));
        end
        G{j} = G{j} / ms;
